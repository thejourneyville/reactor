...
class Ball:
    ...
    def draw_ball(self, color):
        pygame.draw.circle(
            surface, (color, 17, 59), (self.ball.x + self.radius, self.ball.y + self.radius), self.radius, int(15 * scaler))
...
def stats(total_points, lives_left, timer):
    ...
    lives_font = pygame.font.Font(f"./{font_style}", int(40 * scaler))
    ...
    text_surface1 = lives_font.render(f"lives: {lives_left}", True, text_color)
    ...
    text_rect1.centerx, text_rect1.centery = surface_width // 2, surface_height // 2 + (20 * scaler)

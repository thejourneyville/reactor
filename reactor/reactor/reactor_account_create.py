...
def account_create(surface, surface_width, surface_height, margin, margin_color,
    ...
    def text_board():
        ...
        # all_keyboard_range = range(len(keyboard_1 + keyboard_2 + keyboard_3 + keyboard_4))
    ...
    def highlight_text(coord, k_pos, letter):
            ...
            if idx < len(k_pos) - 11:
                ...
                if pos[0] <= x < pos[0] + letter[0]:
                    if pos[-1] <= y < pos[-1] + letter[-1]:
                        if letters[idx] != " ":
                            if 49 <= idx <= 51:
                                highlighted, button, highlighted_pos = idx, "adv", k_pos[49]
                            elif 53 <= idx <= 55:
                                highlighted, button, highlighted_pos = idx, "rub", k_pos[53]
                            elif 57 <= idx <= 59:
                                highlighted, button, highlighted_pos = idx, "end", k_pos[57]
        ...
        if highlighted is not None and highlighted_pos is not None:
            ...
            if highlighted < len(letters) - 11:
                ...
                hovered_let_surface = alphabet.render(button, True, color.alert_red, color.background)
                hovered_let_rect = hovered_let_surface.get_rect()
    ...
    while not return_main_menu:
        ...
        highlight_text(mouse_xy, all_pos, let_size)
